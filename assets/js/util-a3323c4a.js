...
function() {
    ...
    $.confirm = function(t) {
        ...
        e.find(".okbtn").html(i)) : e = $("<div id='global_confirm_window' tabindex='-1' class='confirm-box' title='请确认'><div class='dlg-content'>" + t.content + "</div><div class='dlg-buttons'><span class='pro-btn default okbtn'>" + i + "</span>&nbsp;&nbsp;<span class='pro-btn cancelbtn close'>取消</span></div></div>").appendTo("body"),
            ...
            l.find("input").off().on("keyup", function(e) {
                ...
                if ("" != i)
                    if (13 == e.keyCode && t.setVal) {
                        if (!/^([\w]+)(.[\w]+)*@([\w-]+\.){1,5}([A-Za-z]){2,4}$/.test(i))
                            return;
                        o(null, t.setVal(i)),
                        $(this).val("")
                    } else
                ...
                !($(this).children().length < 1)))
                    ...
                    f) {
                        ...
                        var l = window.getSelection()
                          , s = l.getRangeAt(0);
                        s.deleteContents();
                        var r, d, c = $("<div>" + a + "</div>").html(), h = document.createElement("div"), v = document.createDocumentFragment();
                        if (c)
                            for (h.innerHTML = c; r = h.firstChild; )
                                d = v.appendChild(r),
                        ...
                        d) {
                            ...
                            g && m) {
                                ...
                                r = this.childNodes[w],
                                s.setStart(r, r.data.length - m),
                                s.setEnd(r, r.data.length - m)
                            }
                            l.removeAllRanges(),
                            l.addRange(s)
                        }
